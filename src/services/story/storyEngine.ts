...
const REQUIRED_FINAL_EVIDENCE = ['E-004821', 'E-004823', 'E-004829', 'R-1998-112', 'CASE-1989-114', 'VEH-1987-0481'];
...
class StoryEngine {
  ...
  private createDefaultState(): StoryState {
    return {
      currentAct: 1,
    ...
    try {
      ...
      if (stored) {
        ...
        return {
          ...
          discoveredStepIds: Array.isArray(parsed.discoveredStepIds) ? parsed.discoveredStepIds : [],
          discoveredFactIds: Array.isArray(parsed.discoveredFactIds) ? parsed.discoveredFactIds : [],
          openQuestionIds: Array.isArray(parsed.openQuestionIds) ? parsed.openQuestionIds : [],
          resolvedQuestionIds: Array.isArray(parsed.resolvedQuestionIds) ? parsed.resolvedQuestionIds : [],
          discoveredContradictionIds: Array.isArray(parsed.discoveredContradictionIds) ? parsed.discoveredContradictionIds : [],
          activeLeadIds: Array.isArray(parsed.activeLeadIds) ? parsed.activeLeadIds : [],
          timelineEventIds: Array.isArray(parsed.timelineEventIds) ? parsed.timelineEventIds : [],
  ...
  private notify(): void {
    ...
    this.listeners.forEach((listener) => {
      try { listener(currentState); } catch (e) { console.error('[StoryEngine] Listener error:', e); }
    });
  }
  ...
  private isSearchMatch(clean: string, searchTerm: string): boolean {
    const needle = searchTerm.toLowerCase().trim();
    if (!needle) return false;
    if (needle.includes(' ')) return clean.includes(needle);
  ...
  private executeMatchingSteps(predicate: (step: DiscoveryStep) => boolean): boolean {
    ...
    DISCOVERY_STEPS.forEach((step) => {
      if (predicate(step) && !this.state.discoveredStepIds.includes(step.id)) {
  ...
  public onViewRecord(recordId: string): void {
    ...
    this.executeMatchingSteps((step) =>
      step.act <= this.state.currentAct &&
      step.trigger.type === 'view_record' &&
      Boolean(step.trigger.targetId) &&
  ...
  public onSearchTerm(term: string): void {
    ...
    this.executeMatchingSteps((step) =>
      ...
      Boolean(step.trigger.searchTerm) &&
      this.isSearchMatch(clean, step.trigger.searchTerm!)
    );
  }
  ...
  private areStepPrerequisitesMet(step: DiscoveryStep): boolean {
    return !step.trigger.requiredFlags || step.trigger.requiredFlags.every((flag) => this.hasFlag(flag));
  }
    ...
    do {
      ...
      DISCOVERY_STEPS.forEach((step) => {
        if (
          ...
          this.areStepPrerequisitesMet(step) &&
  ...
  private executeStep(step: DiscoveryStep): void {
    ...
    step.factsAdded.forEach((factId) => {
      if (!this.state.discoveredFactIds.includes(factId)) this.state.discoveredFactIds.push(factId);
    });
    step.questionsAdded?.forEach((qid) => {
      if (!this.state.openQuestionIds.includes(qid) && !this.state.resolvedQuestionIds.includes(qid)) this.state.openQuestionIds.push(qid);
    });
    step.questionsResolved?.forEach((qid) => {
      this.state.openQuestionIds = this.state.openQuestionIds.filter((id) => id !== qid);
      if (!this.state.resolvedQuestionIds.includes(qid)) this.state.resolvedQuestionIds.push(qid);
    });
    step.timelineEventsAdded?.forEach((evtId) => {
      if (!this.state.timelineEventIds.includes(evtId)) this.state.timelineEventIds.push(evtId);
    });
    if (step.contradictionAdded && !this.state.discoveredContradictionIds.includes(step.contradictionAdded)) {
      ...
      this.notificationHandler?.('Timeline Conflict', 'Two records disagree. Compare the sources before deciding what happened.', 'warning');
    }
    step.leadsAdded?.forEach((leadId) => {
      if (!this.state.activeLeadIds.includes(leadId)) this.state.activeLeadIds.push(leadId);
    });
  }

  /**
   * Progression is now milestone-driven. A player must discover the act's explicit
   * synthesis step (when one exists); discovery percentage is only a safety fallback
   * for legacy acts without a synthesis step.
   */
  public checkActProgression(): void {
    if (this.state.currentAct >= 6) return;
    const act = this.state.currentAct;
    const actSteps = DISCOVERY_STEPS.filter((s) => s.act === act);
    ...
    this.notificationHandler?.(
      `Case 27 // ${finishedAct.revelationTitle}`,
      `A new line of inquiry is open: ${STORY_ACTS[nextAct - 1].subtitle}.`,
      'success'
    );
  }
  ...
  public getDiscoveredTimelineEvents(): InvestigationTimelineEvent[] {
    return TIMELINE_EVENTS.filter((evt) => this.state.timelineEventIds.includes(evt.id)).sort((a, b) => `${a.date} ${a.time || ''}`.localeCompare(`${b.date} ${b.time || ''}`));
  }
  ...
  private getActionableStep(): DiscoveryStep | null {
    const currentSteps = DISCOVERY_STEPS.filter((s) => s.act === this.state.currentAct && !this.state.discoveredStepIds.includes(s.id));
    const actionable = currentSteps.find((step) => {
      if (step.trigger.type === 'flag') return this.areStepPrerequisitesMet(step) && Boolean(step.trigger.targetId && this.hasFlag(step.trigger.targetId));
      return true;
    });
    return actionable || currentSteps[0] || null;
  }
  ...
  public getNextInvestigationAction(): InvestigationAction | null {
    ...
    return {
      ...
      targetId: step.trigger.targetId,
      searchTerm: step.trigger.searchTerm,
      hintLevel,
      hintText,
      isOptional: step.order % 10 !== 0
    };
  }
  ...
  public getInvestigationStatus(): {
    ...
    return {
      ...
      whatWeKnow: facts.length ? facts : ['The case was reopened after an unexplained post-closure modification.'],
      unresolvedQuestions: questions.length ? questions : ['What prompted the post-closure modification on Case 27?'],
      activeLeads: leads,
      recentDiscoveriesCount: this.state.discoveredStepIds.length,
      nextAction: this.getNextInvestigationAction()
    };
  }

  public getNextAvailableHint() {
    const action = this.getNextInvestigationAction();
    if (!action) return null;
    return { stepId: action.stepId, stepTitle: action.title, currentLevel: action.hintLevel, hintText: action.hintText };
  }

  public unlockHint(stepId: string): string {
    const step = DISCOVERY_STEPS.find((s) => s.id === stepId);
    if (!step) return 'No investigative lead is available for this thread.';
    const nextLevel = Math.min((this.state.hintsUnlocked[stepId] || 0) + 1, 3);
    this.state.hintsUnlocked[stepId] = nextLevel;
    this.notify();
    return nextLevel === 1 ? step.hintLevel1 : nextLevel === 2 ? step.hintLevel2 : step.hintLevel3;
  }
  ...
  public getDeductionReadiness(): DeductionReadiness {
    ...
    return {
      ...
      reasons,
      discoveredEvidenceIds: REQUIRED_FINAL_EVIDENCE.filter((id) => Boolean(policeDatabase.getRecord(id))),
      requiredEvidenceIds: REQUIRED_FINAL_EVIDENCE
    };
  }
  ...
  public submitDeduction(sub: DeductionSubmission): DeductionResult {
    ...
    // Final determination is evidence-led, not keyword-led. Every field must map to
    // the canonical solution and the selected exhibits must contain the required chain.
    ...
    return {
      ...
      feedback: {
        ...
        when: whenCorrect ? 'Corroborated: September 14, 1998 is the verified incident date.' : 'Contradicted: the selected date conflicts with the reconstructed timeline.',
      ...
      officialDetermination: isFullyCorrect
        ? 'CASE-1998-027 // RECONSTRUCTION ACCEPTED. The evidentiary chain is complete and the case is formally resolved.'
        : `CASE-1998-027 // REMANDED. ${readiness.reasons.length ? readiness.reasons.join(' ') + ' ' : ''}The record does not yet support a complete determination.`
    };
  }
  ...
  public createEvidence(evidenceData: Partial<EvidenceRecord>): EvidenceRecord {
    ...
    const newRecord: EvidenceRecord = {
      ...
      chainOfCustody: evidenceData.chainOfCustody || [{ id: `cust_${Date.now()}`, timestamp: now, action: 'Initial Intake Logging', fromOfficerOrLocation: 'Field Intake', toOfficerOrLocation: 'Evidence Vault', reason: 'Initial storage' }],
  ...
  public updateEvidence(id: string, partial: Partial<EvidenceRecord>): EvidenceRecord | null {
    ...
    if (updated) { this.checkStepTriggers(); this.checkActProgression(); this.notify(); }
  ...
  public revealEvidence(id: string): void {
    ...
    if (rec) { policeDatabase.updateRecord(id, { status: 'IN_STORAGE', currentStatus: 'IN_STORAGE' }); this.onViewRecord(id); }
  }
  ...
  public modifyCustodyEvent(evidenceId: string, eventId: string, partial: Partial<CustodyTransfer>): void {
    ...
    if (rec?.chainOfCustody) { const target = rec.chainOfCustody.find((c) => c.id === eventId); if (target) { Object.assign(target, partial); policeDatabase.updateRecord(evidenceId, { chainOfCustody: [...rec.chainOfCustody] }, 'Auditor', 'Modified custody log'); } }
  }
  public linkEvidence(evidenceId: string, targetType: 'case' | 'person' | 'vehicle' | 'report' | 'location', targetId: string): void {
    const rec = policeDatabase.getRecord(evidenceId) as EvidenceRecord | null; if (!rec) return;
    if (targetType === 'person' && !rec.relatedPersonIds.includes(targetId)) rec.relatedPersonIds = [...rec.relatedPersonIds, targetId];
    else if (targetType === 'vehicle') { rec.relatedVehicleId = targetId; rec.relatedVehicleIds = Array.from(new Set([...(rec.relatedVehicleIds || []), targetId])); }
    else if (targetType === 'case') rec.relatedCaseIds = Array.from(new Set([...(rec.relatedCaseIds || []), targetId]));
    else if (targetType === 'report') rec.relatedReportIds = Array.from(new Set([...(rec.relatedReportIds || []), targetId]));
    else if (targetType === 'location') rec.relatedLocationIds = Array.from(new Set([...(rec.relatedLocationIds || []), targetId]));
  ...
  public unlinkEvidence(evidenceId: string, targetType: 'case' | 'person' | 'vehicle' | 'report' | 'location', targetId: string): void {
    const rec = policeDatabase.getRecord(evidenceId) as EvidenceRecord | null; if (!rec) return;
    if (targetType === 'person') rec.relatedPersonIds = rec.relatedPersonIds.filter((id) => id !== targetId);
    else if (targetType === 'vehicle') { rec.relatedVehicleIds = (rec.relatedVehicleIds || []).filter((id) => id !== targetId); if (rec.relatedVehicleId === targetId) rec.relatedVehicleId = undefined; }
    else if (targetType === 'case') rec.relatedCaseIds = (rec.relatedCaseIds || []).filter((id) => id !== targetId);
    else if (targetType === 'report') rec.relatedReportIds = (rec.relatedReportIds || []).filter((id) => id !== targetId);
    else if (targetType === 'location') rec.relatedLocationIds = (rec.relatedLocationIds || []).filter((id) => id !== targetId);
  ...
  public createBoardConnection(from: string, to: string, label: string, relationType: string = 'associated with', isContradiction = false, isSupport = false): BoardEdge {
    const edge: BoardEdge = { id: `edge_${from}_${to}_${Date.now()}`, from, to, label, relationType, isContradiction, isSupport }; policeDatabase.addBoardEdge(edge);
    const board = policeDatabase.getBoardState(); const source = board.nodes.find((n) => n.id === from); const target = board.nodes.find((n) => n.id === to);
    if (source?.recordId && target?.recordId) { this.setFlag(`connected_${source.recordId}_${target.recordId}`, true); this.setFlag(`connected_${target.recordId}_${source.recordId}`, true); }
    return edge;
  }
