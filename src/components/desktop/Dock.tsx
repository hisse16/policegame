...
export const Dock: React.FC = () => {
  ...
  const currentAct = storyEngine.getState().currentAct;
  ...
  // Keep already-open applications visible so a story action can never strand an open window.
        ...
        {dockAppIds.map((appId) => {
          ...
          return (
            <button
              ...
              className={`relative group flex flex-col items-center justify-center w-10 h-10 rounded-lg transition-all duration-150 ${isActive ? 'bg-slate-800/90 text-blue-400 shadow-inner' : isOpen ? 'bg-slate-900/60 text-slate-200 hover:bg-slate-800/60' : 'text-slate-400 hover:text-slate-100 hover:bg-slate-800/40'}`}
            >
