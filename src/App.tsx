...
const InvestigationGuide: React.FC = () => {
  ...
  const openLead = () => {
    if (action.targetId && (action.actionType === 'view_record' || action.actionType === 'view_file')) {
  ...
  return (
    <div className={`absolute left-4 bottom-16 z-[8000] w-[340px] max-w-[calc(100vw-2rem)] ${collapsed ? 'w-auto' : ''}`}>
      {collapsed ? (
        ...
        <div className="bg-slate-950/95 backdrop-blur border border-slate-700 rounded-xl shadow-2xl overflow-hidden">
          ...
          <div className="p-3 space-y-2.5">
            ...
            <div className="text-[10px] text-slate-500 uppercase tracking-wide">
              You are not being told the answer. This is the next line worth investigating.
            </div>
            <div className="flex gap-2 pt-1">
              <button onClick={openLead} className="flex-1 px-2.5 py-1.5 rounded bg-blue-600 hover:bg-blue-500 text-white text-[11px] font-semibold">
                Open lead
              </button>
              <button onClick={() => openApp('investigation-notebook')} className="px-2.5 py-1.5 rounded bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 text-[11px]">
                Notebook
              </button>
            </div>
          </div>
  ...
  return (
    ...
    >
      ...
      {gameSettings.crtScanlines && (
        <div className="absolute inset-0 pointer-events-none opacity-[0.05] z-9999" style={{ backgroundImage: 'linear-gradient(rgba(18, 16, 16, 0) 50%, rgba(0, 0, 0, 0.45) 50%)', backgroundSize: '100% 3px' }} />
      )}
    </div>
  );
};
