...
// Northbridge is fictional. The story data already gives each location an x/y
// position; those positions are projected onto a real street basemap so the
// player sees a conventional city map rather than a diagram of rectangles.
...
const Compass: React.FC = () => {
  ...
  return (
    <button
      onClick={resetNorth}
      className="absolute right-3 top-[58px] z-[1000] flex h-11 w-11 items-center justify-center rounded-full border border-slate-200 bg-white shadow-[0_2px_10px_rgba(0,0,0,.18)] transition hover:bg-slate-50"
      title="North"
...
export const MapRenderer: React.FC<MapRendererProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-[#e5e7eb] text-slate-900 select-none">
      <style>{`
        .northbridge-map .leaflet-container {
          width:100%;height:100%;background:#dbe7ee;
        ...
        .northbridge-map .leaflet-control-zoom a {
          ...
          background:#fff!important;color:#3c4043!important;font-size:21px!important;
        }
        ...
        .northbridge-map .leaflet-control-attribution {
          font-size:9px!important;background:rgba(255,255,255,.86)!important;
          padding:2px 5px!important;color:#5f6368!important;
        }
        .northbridge-map .leaflet-control-scale-line {
          border:2px solid #5f6368!important;border-top:0!important;
          background:rgba(255,255,255,.75)!important;color:#3c4043!important;
        ...
        .northbridge-map .leaflet-popup-content-wrapper {
          ...
          box-shadow:0 5px 24px rgba(0,0,0,.2)!important;
        }
        .northbridge-map .leaflet-popup-tip { box-shadow:2px 2px 4px rgba(0,0,0,.08)!important; }
        .northbridge-map .leaflet-popup-content { margin:0!important; }
        .northbridge-map .leaflet-tooltip {
          border:0!important;border-radius:6px!important;padding:5px 8px!important;
          box-shadow:0 2px 8px rgba(0,0,0,.18)!important;font-size:11px!important;
          font-weight:600!important;color:#3c4043!important;
        }
        .northbridge-map .leaflet-marker-icon { transition:transform .15s ease; }
      `}</style>
      ...
      {/* Search is intentionally the only large overlay: the map remains the main surface. */}
      <div className="absolute left-4 top-4 z-[1000] w-[min(430px,calc(100%-32px))]">
        <div className="flex h-12 items-center overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_2px_12px_rgba(0,0,0,.18)]">
          ...
          <button
            ...
            aria-label={showPlaces ? 'Hide case locations' : 'Show case locations'}
          >
