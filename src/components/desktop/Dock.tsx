...
const ACT_UNLOCKS: Record<number, string[]> = {
  1: ['police-records', 'file-manager', 'investigation-notebook'],
...
// These are the actual investigation tools. Utilities such as Settings,
// Calculator and System Monitor are intentionally not part of the game flow.
const INVESTIGATION_APPS = [
  'police-records',
  'evidence-lab',
  'investigation-board',
...
export const Dock: React.FC = () => {
  ...
  const unlockedApps = new Set(
    INVESTIGATION_APPS.filter((appId) =>
      Object.entries(ACT_UNLOCKS).some(([act, ids]) => Number(act) <= currentAct && ids.includes(appId))
    )
  );

  // Keep an opened investigation tool visible even if its story unlock changes.
  const openAppIds = Array.from(new Set<string>(windows.map((w) => w.appId))).filter((id) => INVESTIGATION_APPS.includes(id));
  const dockAppIds = Array.from(new Set([...INVESTIGATION_APPS.filter((id) => unlockedApps.has(id)), ...openAppIds]));

  return (
    <div data-os-dock="true" className="fixed bottom-2 left-1/2 -translate-x-1/2 z-40 select-none">
      <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-950/90 backdrop-blur-md border border-slate-700/60 rounded-xl shadow-2xl">
        {dockAppIds.map((appId) => {
          ...
          return (
            <button key={appId} onClick={handleClick} title={appDef.name} aria-label={appDef.name} className={`relative group flex flex-col items-center justify-center w-10 h-10 rounded-lg transition-all duration-150 ${isActive ? 'bg-slate-800/90 text-blue-400 shadow-inner' : isOpen ? 'bg-slate-900/60 text-slate-200 hover:bg-slate-800/60' : 'text-slate-400 hover:text-slate-100 hover:bg-slate-800/40'}`}>
