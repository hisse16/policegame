...
const byText = (text: string) => buttons().find(b => (b.textContent || '').trim() === text) || null;
const top = () => document.querySelector('header') as HTMLElement | null;
const topButton = (n: number) => top()?.querySelector(`:scope > div:last-child > button:nth-child(${n})`) as HTMLElement | null;
const appButton = () => top()?.querySelector(':scope > div:first-child > button') as HTMLElement | null;
const clockButton = () => top()?.querySelector(':scope > div.absolute button') as HTMLElement | null;
const launcher = () => document.querySelector('#os-app-launcher') as HTMLElement | null;
const popupWith = (text: string) => Array.from(document.querySelectorAll('div.fixed')).find(e => (e.textContent || '').includes(text)) as HTMLElement | null;
...
export const WorkstationTutorial: React.FC<WorkstationTutorialProps> = ({ onFinish }) => {
  ...
  const createdWindow = useRef<string | null>(null);
  const createdApp = useRef<string | null>(null);
  ...
  useEffect(() => {
    let stopped = false;
    const oldApp = createdApp.current;
    if (oldApp && oldApp !== step.app && createdWindow.current) {
      closeWindow(createdWindow.current);
      createdWindow.current = null;
      createdApp.current = null;
    }
    if (step.app) {
      const existing = windows.find(w => w.appId === step.app);
      const id = openApp(step.app);
      if (!existing && id) { createdWindow.current = id; createdApp.current = step.app; }
    }
    step.prepare?.();
    ...
    const poll = () => {
      if (stopped) return;
      refresh();
      if (Date.now() - started < 4000) window.setTimeout(poll, 80);
    };
    poll();
    return () => { stopped = true; };
  }, [index, step.app]);
  ...
  const finish = () => {
    if (createdWindow.current) closeWindow(createdWindow.current);
    createdWindow.current = null;
    createdApp.current = null;
    onFinish();
  };
