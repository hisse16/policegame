...
const MAP_CENTER: [number, number] = [MAP_HEIGHT / 2, MAP_WIDTH / 2];

const toMapPoint = (loc: MapLocation): [number, number] => [
  MAP_HEIGHT - loc.y,
  (loc.x / 100) * MAP_WIDTH,
];
...
const markerColor = (category: string) => {
  switch (category) {
    case 'Residential': return '#7c3aed';
    case 'Commercial':
    case 'Business': return '#2563eb';
    case 'Food': return '#ea580c';
    case 'Road': return '#475569';
    case 'Emergency': return '#dc2626';
    case 'Government': return '#15803d';
...
const markerIcon = (category: string, selected = false) => {
  ...
  return L.divIcon({
    ...
    html: `
      <div style="position:relative;width:${size}px;height:${size}px;filter:drop-shadow(0 2px 4px rgba(0,0,0,.30));">
        ${selected ? `<div style="position:absolute;left:50%;top:2px;transform:translate(-50%,-50%);width:46px;height:46px;border-radius:50%;border:2px solid ${color};opacity:.28;"></div>` : ''}
        <div style="position:absolute;left:50%;top:1px;transform:translateX(-50%) rotate(45deg);width:${size - 6}px;height:${size - 6}px;border-radius:50% 50% 50% 0;background:${color};border:2px solid #fff;"></div>
...
const MapReset: React.FC<{ resetKey: number }> = ({ resetKey }) => {
  const map = useMap();
  useEffect(() => {
    if (resetKey > 0) map.fitBounds(MAP_BOUNDS, { padding: [18, 18] });
  }, [map, resetKey]);
  return null;
};
...
const Compass: React.FC = () => (
  <div className="absolute left-3 top-3 z-[1000] flex h-11 w-11 items-center justify-center rounded-full border border-slate-200 bg-white shadow-[0_2px_10px_rgba(0,0,0,.18)]" title="North" aria-label="North">
...
export const MapRenderer: React.FC<MapRendererProps> = ({ onNavigate }) => {
  ...
  const filteredLocations = useMemo(() => {
    ...
    if (!q) return CASE_27_MAP_LOCATIONS;
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-slate-200 text-slate-900 select-none">
      <style>{`
        .northbridge-map .leaflet-container { width:100%;height:100%;background:#dbe3e8;font-family:Inter,ui-sans-serif,system-ui,sans-serif; }
        ...
        .northbridge-map .leaflet-tooltip { border:0!important;border-radius:7px!important;padding:5px 8px!important;box-shadow:0 2px 8px rgba(0,0,0,.18)!important;font-size:11px!important;font-weight:600!important;color:#3c4043!important; }
      `}</style>
      ...
      <div className="northbridge-map absolute inset-0">
        ...
        >
          ...
          {showPlaces && filteredLocations.map(location => (
            ...
            >
              <Tooltip direction="top" offset={[0, -28]} opacity={0.98}>{location.name}</Tooltip>
              <Popup closeButton>
                <div className="w-[280px]">
                  ...
                  <div className="px-4 py-3">
                    ...
                    <button
                      ...
                      className="mt-3 w-full rounded-lg bg-[#1a73e8] px-3 py-2 text-[11px] font-semibold text-white transition hover:bg-[#1765cc]"
                    >
      ...
      <div className="absolute left-4 top-16 z-[1000] w-[min(430px,calc(100%-32px))]">
        <div className="flex h-12 items-center overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_2px_12px_rgba(0,0,0,.18)]">
          ...
          />
          {searchQuery && (
            <button onClick={() => setSearchQuery('')} className="mr-1 rounded-full p-2 text-slate-400 hover:bg-slate-100" aria-label="Clear search">
          ...
          <button
            ...
            className={`mx-1 rounded-lg p-2 ${showPlaces ? 'bg-blue-50 text-blue-600' : 'text-slate-500 hover:bg-slate-100'}`}
            title={showPlaces ? 'Hide case locations' : 'Show case locations'}
            aria-label="Toggle case locations"
          >
        ...
        {searchQuery && (
          <div className="mt-2 max-h-72 overflow-auto rounded-xl border border-slate-200 bg-white shadow-[0_4px_18px_rgba(0,0,0,.18)]">
            {filteredLocations.length === 0 ? (
              ...
              >
                <span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-white" style={{ backgroundColor: markerColor(location.category) }}>
      ...
      <button
        onClick={resetMap}
        className="absolute right-3 top-4 z-[1000] flex h-11 w-11 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-600 shadow-[0_2px_10px_rgba(0,0,0,.18)] hover:bg-slate-50"
        title="Recenter map"
