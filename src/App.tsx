...
const getActProgress = (state: StoryState) => {
  ...
  return { completed: actSteps.filter((step) => state.discoveredStepIds.includes(step.id)).length, total: actSteps.length };
};
const getFallbackAction = (state: StoryState): InvestigationAction | null => {
  ...
  if (!step) return null;
  return { stepId: step.id, title: step.title, description: step.description, actionType: step.trigger.type, targetId: undefined, searchTerm: step.trigger.searchTerm, hintLevel: 1, hintText: step.hintLevel1, isOptional: false };
};
